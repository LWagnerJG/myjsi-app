...
export const RequestComYardageScreen = ({ theme, showAlert, onNavigate, userSettings }) => {
    ...
    const fabricStrings = useMemo(() =>
        FABRICS_DATA.map(f => `${f.supplier}, ${f.pattern}`), []);

    const modelOptions = useMemo(() =>
        JSI_MODELS.filter(m => m.isUpholstered)
            .map(m => `${m.name} (${m.id})`), []);
    ...
    const addModel = (e) => {
        ...
        const model = JSI_MODELS.find(m => m.id === modelId);
        if (!model) return;
                ...
                <GlassCard theme={theme} className="p-4">
                    ...
                    {selectedModels.map(m => (
                        <GlassCard key={m.key} theme={theme} className="p-4 mb-3 space-y-3">
                            ...
                            <StyledSelect
                                value={m.fabric}
                                onChange={e => updateModel(m.key, { fabric: e.target.value })}
