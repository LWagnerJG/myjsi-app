...
export const ContactSearchSelector = ({ value, onChange, dealers, theme, multiple = false }) => {
  ...
  const SearchRow = (
    <div className="flex h-10 items-center gap-2 px-3" style={{ background: fieldBg, borderRadius: '12px' }}>
      <Search size={13} aria-hidden="true" style={{ color: theme.colors.textSecondary, opacity: 0.4, flexShrink: 0 }} />
      <input
        value={query}
        role="combobox"
        aria-expanded={open}
        ...
        className="flex-1 min-w-0 bg-transparent outline-none text-[0.8125rem] font-medium"
  ...
  const SelectedCard = (name) => (
    <div key={name} className="flex h-10 items-center gap-2 px-3" style={{ background: fieldBg, borderRadius: '12px' }}>
      <div className="w-6 h-6 rounded-full flex items-center justify-center text-[0.625rem] font-bold flex-shrink-0" style={{ backgroundColor: `${theme.colors.accent}16`, color: theme.colors.accent }}>
        {getInitials(name)}
      </div>
      <span className="flex-1 text-[0.8125rem] font-medium truncate" style={{ color: theme.colors.textPrimary }}>{name}</span>
      <button type="button" onClick={() => removeContact(name)} className="w-6 h-6 rounded-full flex items-center justify-center opacity-40 hover:opacity-100 transition-opacity focus-ring" aria-label={`Remove ${name}`}>
        <X className="w-3 h-3" aria-hidden="true" style={{ color: theme.colors.textSecondary }} />
      </button>
    </div>
  );
