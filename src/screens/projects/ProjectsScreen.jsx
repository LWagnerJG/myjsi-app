            ...
            {(() => {
              ...
              return cta && (
                <button
                  ...
                  className="h-8 inline-flex items-center justify-center gap-1 rounded-full text-xs font-semibold transition-all px-3 whitespace-nowrap active:scale-[0.97]"
          ...
          {projectsTab==='customers' && (
            MOCK_CUSTOMERS.length ? (
                      ...
                      <div className="rounded-2xl overflow-hidden transition-all duration-200 hover:-translate-y-0.5 active:scale-[0.98]"
                        ...
                        <div className="relative aspect-video w-full">
                          ...
                          <div className="absolute bottom-0 left-0 right-0 p-3.5">
                            <h3 className="text-[0.9375rem] font-bold text-white tracking-tight leading-snug">{cust.name}</h3>
                            <div className="flex items-center gap-1 mt-0.5">
                              <MapPin className="w-2.5 h-2.5 text-white/80" />
                              <span className="text-[0.6875rem] text-white/80">{cust.location.city}, {cust.location.state}</span>
                            </div>
                            <div className="flex items-center gap-1.5 mt-2">
                              {activeStds > 0 && (
                                <span className="text-[0.625rem] font-bold px-2 py-0.5 rounded-full" style={{ backgroundColor: `${JSI_COLORS.success}40`, color: JSI_COLORS.success }}>
                                  {activeStds} standard{activeStds !== 1 ? 's' : ''}
                                </span>
                              )}
                              {currentOrd > 0 && (
                                <span className="text-[0.625rem] font-bold px-2 py-0.5 rounded-full" style={{ backgroundColor: `${JSI_COLORS.info}40`, color: JSI_COLORS.info }}>
                                  {currentOrd} order{currentOrd !== 1 ? 's' : ''}
                                </span>
                              )}
                              <span className="text-[0.625rem] font-semibold px-2 py-0.5 rounded-full" style={{ backgroundColor: 'rgba(255,255,255,0.15)', color: 'rgba(255,255,255,0.7)' }}>
                                {cust.vertical}
                              </span>
                            </div>
                          </div>
              ...
              <div className="flex flex-col items-center justify-center py-16">
                ...
                <p className="text-center text-[0.9375rem] font-semibold" style={{ color: theme.colors.textPrimary }}>No customers yet</p>
                <p className="text-center text-[0.8125rem] mt-1" style={{ color: theme.colors.textSecondary }}>Customer accounts will appear here</p>
              </div>
            )
          )}
