      ...
      {tabs.map(t => {
        ...
        return (
          <button key={t} onClick={() => onChange(t)}
            className="px-2.5 py-[5px] rounded-full text-[0.6875rem] font-semibold transition-all active:scale-[0.97]"
            style={{
...
const StatusBadge = ({ status }) => {
  ...
  return (
    <span className="text-[0.625rem] font-bold px-2 py-0.5 rounded-full" style={{ backgroundColor: `${color}20`, color }}>{status}</span>
  );
};

const CodeChip = ({ code, theme }) => (
  <span className="text-[0.625rem] font-mono font-bold px-2 py-0.5 rounded-full"
      ...
      <motion.div className="fixed inset-0 flex items-start justify-center p-4 overflow-y-auto"
        ...
        <motion.div className="w-full max-w-lg rounded-2xl my-4 overflow-hidden"
          ...
          <div className="p-4 space-y-4 max-h-[70vh] overflow-y-auto">
            <div>
              <h3 className="text-[0.9375rem] font-bold" style={{ color: c.textPrimary }}>{program.title}</h3>
              <p className="text-xs mt-0.5" style={{ color: c.textSecondary }}>{customer.name} · {customer.location.city}, {customer.location.state}</p>
            </div>
            ...
            {program.poRequirementText && (
              <div className="rounded-xl px-3.5 py-3 flex items-start gap-2.5" style={{ backgroundColor: `${c.accent}0D`, border: `1px solid ${c.accent}30` }}>
                ...
                <div>
                  <p className="text-[0.625rem] font-bold uppercase tracking-wider mb-0.5" style={{ color: c.accent }}>PO Requirement</p>
                  <p className="text-xs leading-snug" style={{ color: c.textPrimary }}>{program.poRequirementText}</p>
                </div>
              </div>
            )}
            ...
            {/* Overview */}
            <div>
              <p className="text-[0.625rem] font-bold uppercase tracking-wider mb-1.5" style={{ color: c.textSecondary, opacity: 0.6 }}>Program Overview</p>
              <p className="text-xs leading-relaxed mb-2" style={{ color: c.textPrimary }}>{program.summary}</p>
            ...
            {program.specialNotes?.length > 0 && (
              <div>
                <p className="text-[0.625rem] font-bold uppercase tracking-wider mb-1.5" style={{ color: c.textSecondary, opacity: 0.6 }}>What's Special</p>
                <ul className="space-y-1">
                  {program.specialNotes.map((note, i) => (
                    <li key={i} className="flex items-start gap-2 text-xs" style={{ color: c.textPrimary }}>
            ...
            {program.purchasingAwarenessRequired && (
              <div>
                <p className="text-[0.625rem] font-bold uppercase tracking-wider mb-1" style={{ color: c.textSecondary, opacity: 0.6 }}>Purchasing Visibility</p>
...
export const CustomerMicrositeScreen = ({ customer, theme, onBack }) => {
  ...
  const renderProgramRow = useCallback((prog, dim = false) => (
    <button key={prog.id} type="button" onClick={() => setSelectedProgram(prog)}
      ...
      <span className="text-xs font-semibold flex-1 truncate" style={{ color: c.textPrimary }}>{prog.title}</span>
          ...
          <div className="flex-1 min-w-0 space-y-3">
            ...
            <div className="flex items-start gap-3.5">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-1.5 mt-0.5">
                  ...
                  <span className="text-xs" style={{ color: c.textSecondary }}>{customer.location.city}, {customer.location.state}</span>
                  <span className="text-[0.625rem] font-semibold px-2 py-0.5 rounded-full ml-1"
            ...
            <Sect title="Orders" theme={theme}>
              ...
              {ordersList.length > 0 ? ordersList.map(o => (
                <div key={o.id} className="flex items-center gap-2.5 py-2 border-t" style={{ borderColor: border }}>
                  <div className="flex-1 min-w-0">
                    <span className="text-xs font-semibold" style={{ color: c.textPrimary }}>{o.orderNumber}</span>
                    <span className="text-[0.6875rem] ml-2" style={{ color: c.textSecondary }}>{fmtMoney(o.amount)}</span>
                  </div>
                  <span className="text-[0.6875rem]" style={{ color: c.textSecondary }}>{o.eta ? `ETA ${fmtDate(o.eta)}` : fmtDate(o.completedAt)}</span>
                  <StatusBadge status={o.status} />
                </div>
              )) : (
                <p className="text-xs py-3 text-center" style={{ color: c.textSecondary, opacity: 0.6 }}>No {orderTab.toLowerCase()} orders</p>
              )}
            </Sect>
                ...
                <div className="space-y-0">
                  {materialsList.map(m => (
                    <div key={m.id} className="flex items-center gap-2.5 py-2 border-t" style={{ borderColor: border }}>
                      ...
                      <div className="flex-1 min-w-0">
                        <span className="text-xs font-semibold" style={{ color: c.textPrimary }}>{m.name}</span>
                        <span className="text-[0.625rem] ml-1.5 font-mono" style={{ color: c.textSecondary }}>{m.code}</span>
                      </div>
                      {m.vendor && <span className="text-[0.6875rem]" style={{ color: c.textSecondary }}>{m.vendor}</span>}
                    </div>
                  ))}
                </div>
              ) : (
                <p className="text-xs py-3 text-center" style={{ color: c.textSecondary, opacity: 0.6 }}>None approved</p>
              )}
            </Sect>
            ...
            {(customer.standardsPrograms || []).length > 0 && (
              <Sect title="Standards & Programs" theme={theme}>
                ...
                {contracts.length > 0 && (
                  <>
                    <p className="text-[0.5625rem] font-bold uppercase tracking-wider mt-2 mb-1" style={{ color: c.accent, opacity: 0.5 }}>Contracts</p>
                    {contracts.map(p => renderProgramRow(p))}
                  </>
                )}
                {dimmed.length > 0 && (
                  <>
                    <p className="text-[0.5625rem] font-bold uppercase tracking-wider mt-2 mb-1" style={{ color: c.textSecondary, opacity: 0.4 }}>Expiring / Expired</p>
                  ...
                  {customer.typicals.map(t => (
                    <div key={t.id} className="rounded-xl overflow-hidden" style={{ border: `1px solid ${border}`, backgroundColor: c.surface }}>
                      ...
                      <div className="px-2.5 py-2">
                        <p className="text-[0.6875rem] font-semibold truncate" style={{ color: c.textPrimary }}>{t.name}</p>
                        <p className="text-[0.625rem]" style={{ color: c.textSecondary }}>{t.dimensions}</p>
                        <p className="text-[0.6875rem] font-bold mt-0.5" style={{ color: c.accent }}>From {fmtMoney(t.startingPrice)}</p>
                      </div>
                    </div>
                  ))}
                </div>
                  ...
                  {(customer.projects || []).map(proj => {
                    ...
                    return (
                      <button key={proj.id} type="button" onClick={() => setSelectedProjectFilter(selectedProjectFilter === proj.id ? 'All' : proj.id)}
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-xs font-semibold truncate" style={{ color: c.textPrimary }}>{proj.name}</p>
                          <p className="text-[0.625rem]" style={{ color: c.textSecondary }}>{proj.location}{photoCount > 0 ? ` · ${photoCount} photo${photoCount !== 1 ? 's' : ''}` : ''}</p>
                        </div>
                        <ChevronRight className="w-3.5 h-3.5 shrink-0 opacity-30" style={{ color: c.textSecondary }} />
                      </button>
                    );
            ...
            {allInstalls.length > 0 && (
              <Sect title={selectedProjectFilter !== 'All' ? `Installs · ${(customer.projects || []).find(p => p.id === selectedProjectFilter)?.name || ''}` : 'Install Gallery'} theme={theme}
                right={selectedProjectFilter !== 'All' && (
                  <button onClick={() => setSelectedProjectFilter('All')} className="text-[0.625rem] font-semibold" style={{ color: c.accent }}>Show All</button>
                )}>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2">
                  {filteredInstalls.map(i => (
                    <button key={i.id} type="button" onClick={() => setLightboxImg(i)}
                      ...
                      <p className="absolute bottom-0 left-0 p-2 text-[0.625rem] text-white font-medium opacity-0 group-hover:opacity-100 transition-opacity truncate">{i.caption}</p>
                    </button>
                  ))}
                </div>
              </Sect>
              ...
              <Sect title="Documents" theme={theme}>
                {customer.documents.map(d => (
                  <div key={d.id} className="flex items-center gap-2.5 py-2 border-t" style={{ borderColor: border }}>
                    ...
                    <span className="text-xs font-medium flex-1 truncate" style={{ color: c.textPrimary }}>{d.name}</span>
              ...
              <Sect title="Contacts" theme={theme}>
                {customer.contacts.filter(ct => ct.visibility === 'dealer').map(ct => (
                  <div key={ct.id} className="flex items-center gap-2.5 py-2 border-t" style={{ borderColor: border }}>
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-semibold" style={{ color: c.textPrimary }}>{ct.name}</p>
                      <p className="text-[0.6875rem]" style={{ color: c.textSecondary }}>{ct.role}</p>
                    </div>
                    <a href={`mailto:${ct.email}`} className="text-[0.6875rem] font-medium" style={{ color: c.accent }}>{ct.email}</a>
                  </div>
                ))}
              </Sect>
            )}
              ...
              <Sect title="At a Glance" theme={theme}>
                <div className="space-y-2.5">
                  {[
                    ...
                    <div key={label} className="flex items-center justify-between">
                      ...
                      <span className="text-[0.8125rem] font-bold tabular-nums" style={{ color: c.textPrimary }}>{val}</span>
                    </div>
                  ))}
                </div>
                ...
                {customer.jsiRep && (
                  <div className="mt-3 pt-3 border-t" style={{ borderColor: border }}>
                    <p className="text-[0.5625rem] font-bold uppercase tracking-wider mb-2" style={{ color: c.textSecondary, opacity: 0.5 }}>Your JSI Rep</p>
                    <div className="flex items-center gap-2.5">
                      <InitialsAvatar name={customer.jsiRep.name} size={32} theme={theme} />
                      <div className="min-w-0">
                        <p className="text-xs font-semibold" style={{ color: c.textPrimary }}>{customer.jsiRep.name}</p>
                        <p className="text-[0.625rem]" style={{ color: c.textSecondary }}>{customer.jsiRep.role}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-3 mt-2">
                      <a href={`mailto:${customer.jsiRep.email}`} className="flex items-center gap-1 text-[0.6875rem] font-medium" style={{ color: c.accent }}>
                        <Mail className="w-3 h-3" /> Email
                      </a>
                      <a href={`tel:${customer.jsiRep.phone}`} className="flex items-center gap-1 text-[0.6875rem] font-medium" style={{ color: c.accent }}>
      ...
      {lightboxImg && createPortal(
        <div className="fixed inset-0 flex items-center justify-center p-4"
          ...
          <div className="absolute bottom-6 left-0 right-0 text-center">
            <p className="text-white text-[0.8125rem] font-medium">{lightboxImg.caption}</p>
            {lightboxImg.projectName && <p className="text-white/60 text-[0.6875rem] mt-0.5">{lightboxImg.projectName}</p>}
            {lightboxImg.date && <p className="text-white/50 text-[0.6875rem] mt-0.5">{fmtDate(lightboxImg.date)}</p>}
          </div>
        </div>,
        document.body,
      )}
