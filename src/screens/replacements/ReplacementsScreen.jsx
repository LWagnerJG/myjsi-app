...
export const ReplacementsScreen = ({ theme }) => {
    ...
    const detectLoop = useCallback(()=> { intervalRef.current = setInterval(()=> { if(videoRef.current && canvasRef.current && videoRef.current.readyState === videoRef.current.HAVE_ENOUGH_DATA){ const v=videoRef.current; const c=canvasRef.current; const ctx=c.getContext('2d'); c.width=v.videoWidth; c.height=v.videoHeight; ctx.drawImage(v,0,0,c.width,c.height); const img=ctx.getImageData(0,0,c.width,c.height); const code=jsQR(img.data,c.width,c.height); if(code){ stopScanning(); const qrData = code.data || ''; const soMatch = qrData.match(/SO[-\s]?\d+/i); const lineMatch = qrData.match(/(?:line|item|li)[-\s:]?\s*(\d+)/i); setFormData({ salesOrder: soMatch ? soMatch[0] : '', lineItem: lineMatch ? lineMatch[1] : '', dealer:'', notes: `Scanned QR: ${qrData}`, photos:[] }); goToForm(); } } }, 320); }, [stopScanning, goToForm]);
