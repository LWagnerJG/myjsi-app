    ...
    return (
        <div className="flex items-center gap-3 py-2.5">
            ...
            <div className="flex-1 min-w-0">
                <p className="font-medium truncate text-[13px] leading-tight" style={{ color: theme.colors.textPrimary }}>{item.name}</p>
                {item.code && <p className="text-[11px] mt-0.5" style={{ color: theme.colors.textSecondary, opacity: 0.7 }}>{item.code}</p>}
            </div>
            <div className="flex items-center gap-0.5">
                ...
                <span className="font-semibold w-5 text-center text-[13px] tabular-nums" style={{ color: theme.colors.textPrimary }}>{item.quantity}</span>
