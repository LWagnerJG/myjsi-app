...
const SegmentedTabs = ({ theme, active, onChange }) => {
    ...
    const recalc = useCallback(() => {
        ...
        if (i === -1) {
            setU({ left: 0, width: 0, ready: false });
            return;
        }
            ...
            {tabs.map((t, i) => {
                ...
                return (
                    <button
                        ...
                        className="flex-1 flex items-center justify-center gap-2 h-12 font-semibold text-sm tracking-wide"
            ...
            {data.map((item, idx) => {
                ...
                return (
                    <div key={item.month} className="grid grid-cols-[3rem,1fr,auto] items-center gap-x-4 text-sm">
                        <span className="font-semibold" style={{ color: theme.colors.textSecondary }}>
                            {item.month}
                        </span>
...
const CustomerMonthlyBreakdown = ({ monthData, orders, theme, onBack }) => {
    ...
    const customerData = useMemo(() => {
        ...
        monthlyOrders.forEach((o) => {
            if (!map[o.company]) map[o.company] = { company: o.company, bookings: 0 };
    ...
    return (
        <div>
            <button
                onClick={onBack}
                className="flex items-center space-x-2 text-sm font-semibold mb-4"
        ...
        <Modal show={!!order} onClose={onClose} title={`PO #${order?.po}`} theme={theme}>
            <div className="space-y-4 text-sm" style={{ color: theme.colors.textPrimary }}>
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div>
                        <div className="font-semibold" style={{ color: theme.colors.textSecondary }}>
                            Order Date
                        </div>
                ...
                <div>
                    <h4 className="font-bold border-t pt-3 mt-3" style={{ borderColor: theme.colors.subtle }}>
                        Line Items
                    </h4>
...
export const SalesScreen = ({ theme, onNavigate }) => {
    ...
    // Trend info popover state/positioning
    ...
    const { totalBookings, totalSales } = useMemo(() => {
        const bookings = MONTHLY_SALES_DATA.reduce((a, m) => a + m.bookings, 0);
        const sales = MONTHLY_SALES_DATA.reduce((a, m) => a + m.sales, 0);
    ...
    const salesByVertical = useMemo(() => {
        if (SALES_VERTICALS_DATA?.length)
            return SALES_VERTICALS_DATA.map((v) => ({ name: v.label || v.vertical, value: v.value, color: v.color }));
        const verticalSales = {};
        ORDER_DATA.forEach((o) => {
            if (o.vertical && o.net) verticalSales[o.vertical] = (verticalSales[o.vertical] || 0) + o.net;
        });
    ...
    // Close trend info when clicking outside (supports portal)
    useEffect(() => {
        if (!showTrendInfo) return;
        const handler = (e) => {
            const inButton = trendRef.current && trendRef.current.contains(e.target);
            const inPopover = popoverRef.current && popoverRef.current.contains(e.target);
            if (!inButton && !inPopover) setShowTrendInfo(false);
        };
        const onReposition = () => {
            if (!trendRef.current) return;
            const r = trendRef.current.getBoundingClientRect();
            setTrendPos({ top: r.bottom + 8, right: Math.max(8, window.innerWidth - r.right) });
        };
        ...
        <div className="flex flex-col h-full">
            ...
            <div ref={scrollRef} onScroll={handleScroll} className="flex-1 overflow-y-auto scrollbar-hide">
                <div className="px-4 space-y-4 pt-2 pb-4 max-w-6xl mx-auto">
                    <div className="p-6 rounded-[2.5rem] shadow-sm border" style={{ backgroundColor: theme.colors.surface, borderColor: theme.colors.border }}>
                        ...
                        <div className="relative w-full h-6 rounded-full mb-2" style={{ backgroundColor: theme.colors.border }}>
                            ...
                            {(() => {
                                ...
                                const safe = Math.max(percentToGoal, 5);
                                return (
                                    <>
                                        <span
                                            className="absolute top-1/2 -translate-y-1/2 font-bold text-sm px-1"
                                            style={{ left: `${safe}%`, transform: 'translate(-100%, -50%)', color: '#fff' }}
                                        >
                                            ${current}M
                                        </span>
                                        <span
                                            className="absolute top-1/2 -translate-y-1/2 font-semibold text-sm"
                                            style={{ right: '2%', color: theme.colors.textSecondary }}
                                        >
                                            ${goalM}M
                                        </span>
                                    </>
                            ...
                            <>
                                <div className="flex justify-between items-start gap-4 flex-wrap">
                                    <div className="flex flex-col">
                                        ...
                                        <p className="text-base font-semibold mt-3" style={{ color: theme.colors.textSecondary }}>
                        ...
                        <div>
                            {displayedRecent.map((order, i) => (
                                ...
                                >
                                    ...
                                    <p className="font-semibold text-sm mb-2 truncate" style={{ color: theme.colors.textPrimary }}>
                    ...
                    <div className="p-6 rounded-[2.5rem] shadow-sm border" style={{ backgroundColor: theme.colors.surface, borderColor: theme.colors.border }}>
                        <h3 className="font-bold text-xl mb-4" style={{ color: theme.colors.textPrimary }}>
                ...
                {showTrendInfo && createPortal(
                    ...
                    >
                        <p className="font-semibold mb-1" style={{ color: theme.colors.textPrimary }}>
                            {deltaVsLinear >= 0 ? 'Ahead of pace' : 'Behind pace'} {deltaLabel}
                        </p>
                        <p style={{ color: theme.colors.textSecondary, lineHeight: '1.25rem' }}>
                            Goal: {percentToGoal.toFixed(1)}%
                            <br />
