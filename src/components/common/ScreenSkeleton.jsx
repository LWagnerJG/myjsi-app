...
export const ScreenSkeleton = ({ theme }) => {
    ...
    const bg = theme?.colors?.background || (dark ? '#161616' : '#FCFAF7');

    // Match GlassCard raised surface — no light-mode decorative border
    const cardSurface = dark ? 'rgba(255,255,255,0.08)' : (theme?.colors?.surface || '#FFFFFF');
    const chromeWell = dark ? 'rgba(255,255,255,0.06)' : (theme?.colors?.chrome || '#F0EDE8');
    ...
    return (
        ...
        >
            ...
            <div className="h-14 rounded-full mb-4" style={{ ...shimmer, backgroundColor: chromeWell }} />
